...
if (!builder.Environment.IsEnvironment("Testing"))
{
    ...
    builder.Services.AddHostedService<LeaverRetentionBackgroundService>();
}
...
using (var scope = app.Services.CreateScope())
{
    ...
    // Dev/local-only seed data (a named cast covering every edge case) — see
    // DevDataSeeder's own doc comment. Deliberately gated to Development
    // specifically (not just "not Production" like the dev auth scheme/endpoint
    // below) — integration tests run in a "Testing" environment against a fresh
    // database per test class and assert on an otherwise-empty People table, so
    // seeding there would corrupt every test's fixture.
