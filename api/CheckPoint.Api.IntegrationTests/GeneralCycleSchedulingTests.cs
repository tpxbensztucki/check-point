...
public class GeneralCycleSchedulingTests : IAsyncLifetime
{
    ...
    private static FeedbackCycleService CreateService(CheckPointDbContext context, DateTimeOffset now, int skipThresholdWeeks = 4) =>
        new(context, new FakeTimeProvider(now), Options.Create(new GeneralCycleOptions { SkipThresholdWeeks = skipThresholdWeeks }));
